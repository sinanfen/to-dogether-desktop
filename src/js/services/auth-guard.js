...
class AuthGuard {
  ...
  async validateTokens() {
    try {
      ...
      // Initialize app after successful token validation
  ...
  showApp() {
    ...
    // Update user info after a short delay to ensure DOM is ready
    setTimeout(() => {
      if (this.currentUser) {
        console.log('Updating user info after showing app for user:', this.currentUser);
        this.updateUserInfo();
      }
    }, 100);
  }
  ...
  async handleLogin() {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update user info after app initialization
        this.updateUserInfo();
      } else {
  ...
  async handleRegister() {
    ...
    const hasInviteCode =
      document.getElementById("hasInviteCode")?.checked || false;
    const inviteCode = hasInviteCode
    ...
    try {
      ...
      this.showError(
        errorDiv,
        "Kayıt başarısız. Kullanıcı adı zaten kullanılıyor olabilir."
      );
    } finally {
      this.setLoading(false, buttonText, spinner);
    }
  ...
  updateUserInfo() {
    if (this.currentUser) {
      console.log('Updating user info in UI for:', this.currentUser);
      // Update header with user info
      const userInfoDiv = document.getElementById("headerUserInfo");
      if (userInfoDiv) {
        userInfoDiv.innerHTML = `<span>Hoş geldin, ${this.currentUser.username}</span>`;
        console.log('User info updated in header:', this.currentUser.username);
      } else {
        console.warn('headerUserInfo div not found');
      }
    } else {
      console.warn('No current user available for updateUserInfo');
    }
  }
  ...
  // This method was removed to prevent duplicate app.init() calls
}
