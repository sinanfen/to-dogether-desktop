...
class PartnerCard {
  ...
  create() {
    ...
    cardElement.className = `card cursor-pointer transform hover:scale-105 transition-all duration-300 group`;
      ...
      <div class="text-center space-y-4">
        ...
        <div>
          <div class="flex items-center justify-center space-x-2">
            <h3 class="text-xl font-semibold text-gray-900 partner-name" data-partner-id="${this.partner.id}">${this.escapeHtml(this.partner.name)}</h3>
            <button class="edit-name-btn opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-md">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
              </svg>
            </button>
          </div>
          <p class="text-gray-500 text-sm">Partner ${this.partner.id === 'partner1' ? '1' : '2'}</p>
        </div>
  ...
  // Update stats
  updateStats(stats) {
    const statsContainer = this.element.querySelector('.flex.justify-center.space-x-2');
    if (statsContainer) {
  ...
  showEditNameModal() {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">İsim Düzenle</h3>
        
        <form id="editNameForm" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">İsim</label>
            <input type="text" id="editPartnerName" value="${this.escapeHtml(this.partner.name)}" class="input" required>
          </div>
  ...
  addContextMenu() {
    this.element.addEventListener('contextmenu', (e) => {
      ...
      contextMenu.className = 'fixed bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50';
      contextMenu.style.left = e.pageX + 'px';
      contextMenu.style.top = e.pageY + 'px';
      
      contextMenu.innerHTML = `
        <button class="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center">
